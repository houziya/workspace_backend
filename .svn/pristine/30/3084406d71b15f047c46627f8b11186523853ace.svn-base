...
class PublicAction extends CommonAction
{
    ...
    public function homepage2()
    {
        $aaa = A('Mavro');
        $aaa->matchBuySell();
        //$this->redirect('__APP__/Public/homepage',1);
    ...
    public function main()
    {
        ...
        $this->assign('m_r_money', $m_r_money);
        $this->assign('r_money', $r_money);
        $this->assign('y_r_money', $y_r_money);
...
//提现冻结时间
        ...
        $xx01 = $fck->where("re_id=" . $id . " and jingli=0 and zongjian=0 and dongshi=0 and h4=0 and h5=0 and h6=0")->count();
        $xx12 = $fck->where("re_id=" . $id . " and jingli=1")->count();
        $xx23 = $fck->where("re_id=" . $id . " and zongjian=1")->count();
        $xx34 = $fck->where("re_id=" . $id . " and dongshi=1")->count();
        $xx45 = $fck->where("re_id=" . $id . " and h4=1")->count();
        $xx56 = $fck->where("re_id=" . $id . " and h5=1")->count();
  ...
  */
        ...
        $tg = __APP__ . "/Reg/us_reg/rid/" . $_SESSION[C('USER_AUTH_KEY')] . "?p=" . $fck_rs['user_id'];
        $this->assign('tg', $tg);
        $cash = M('cash');
        //求购信息
        //查询字段
        ...
        $map['bid'] = $id;
        //$map['bz']   = array('eq', "");
        //$map['rdt'] = array('egt', strtotime("-18 days"));
        ...
        // $fwhere = array();
        //  $fwhere['ID'] = $_SESSION[C('USER_AUTH_KEY')];
        $fee_brs = $fee->where('id>0')->field('*')->find();
        $cash = M('cash');
        $where = "is_pay=1 and type=0 and is_cancel=0 and is_get=0 and is_timeout=0";
        $cash_brs = M('cash')->where($where)->field('*')->order('id asc')->find();
        if ($cash_brs) {
            $b_id = $cash_brs['id'];
            $match_id = $cash_brs['match_id'];
            $user_id = $cash_brs['user_id'];
            $b_user_id = $cash_brs['b_user_id'];
            if ($cash_brs['pdt'] + 3600 * $fee_brs['s12'] < time()) {
                if ($user_id == $b_user_id) {
                    //$cash->execute("update __TABLE__ set is_timeout=1,is_pay=0 where b_user_id='".$user_id."' and is_done=0 and id=".$b_id);
                    //$cash->execute("update __TABLE__ set is_timeout=1, is_pay=0 where b_user_id='".$user_id."' and is_done=0 and id=".$match_id);
                    $cash->execute("update __TABLE__ set is_timeout=1 where b_user_id='" . $user_id . "' and is_done=0 and id=" . $b_id);
                    $cash->execute("update __TABLE__ set is_timeout=1 where b_user_id='" . $user_id . "' and is_done=0 and id=" . $match_id);
                    //$fck->execute("update __TABLE__ set is_lock=1 where user_id='".$user_id."'");
                    //$this->autoMenberLock($user_id);
                }
            }
        }
        unset($map);
    ...
    /*
                    ...
                    {
                        $cash = M('cash');
                        $rs_cash=M('cash')->where("money=1000 and is_cancel=0 and pdt>1469235600")->select();
                        foreach ($rs_cash as $too) {
                            if($too['type']==0){
                                ...
                                if($too['is_pay']==1&&$too['is_get']==0){
                                    ...
                                    //清空自己
                                    $cash->execute("update __TABLE__ set sid=0,s_user_id=0,is_pay=0,pdt=0,match_id=0,match_num=0 where id=".$too['id']);
                                }
                            }
                            else{
                                ...
                                if($too['is_pay']==1&&$too['is_done']==0){
                                    //找到对应的买方
                                    $match_buy=M('cash')->where("id=".$too['match_id'])->find();
                                    //如果是一对一
                                    ...
                                    //清空自己
                                    $cash->execute("update __TABLE__ set bid=0,b_user_id=0,is_pay=0,pdt=0,match_id=0 where id=".$too['id']);
...
//抢单功能待完善
    public function Qiangdan00()
    {
        ...
        $sname = $_SERVER['SERVER_NAME'];
        // $tg = "http://m5.zzqun.net".__APP__."/";
        $tg = "mmb.axauto.cn" . __APP__ . "/Reg/us_reg/rid/" . $_SESSION[C('USER_AUTH_KEY')] . "?p=" . $fck_rs['user_id'];
        $this->assign('tg', $tg);
        $cash = M('cash');
        //求购信息
        //查询字段
...
*/
    ...
    public function mx()
    {
        ...
        foreach($yixi as $buy){
            if($buy["type"] == 0) {
                if($buy["is_split"] == 1) {
                    $subMatchIds = $cash->where(["parent_id" => $buy["id"]])->field("x1,bz,match_id,sid,bid")->select();
                    if ($subMatchIds){
                        foreach ($subMatchIds as $subInOrder) {
                            //$buy['sid'] = $buy["sid"] . "," . $subInOrder['sid'];
                            $buy['match_id'] = $buy["match_id"] . "," . $subInOrder['match_id'];
                        }
                    }
                }
            }
            if($buy["type"] == 3){
                for($i = 0; $i < 10; $i++){
                    if($i == 0){
                        $subMatchIds = $cash->where(["bz"=>$buy["x1"]])->field("x1,bz,match_id,sid,bid")->find();
                        if($subMatchIds){
                            if(!empty($buy["sid"])){
                                $buy['sid'] = $buy["sid"] . "," . $subMatchIds['sid'];
                            }
                            if(!empty($buy["match_id"])){
                                $buy['match_id'] = $buy["match_id"] . "," . $subMatchIds['match_id'];
                            }
                        }
                    }else{
                ...
                for($i = 0; $i < 10; $i++){
                    if($i == 0){
                        ...
                        if($subMatchIds){
                            if(!empty($buy["bid"])){
                                $buy['bid'] = $buy["bid"] . "," . $subMatchIds['bid'];
                            }
                            if(!empty($buy["match_id"])){
                                $buy['match_id'] = $buy["match_id"] . "," . $subMatchIds['match_id'];
                            }
    ...
    public function buys()
    {
        ...
        if (empty($vo['zhifuPay'])) {
            $this->error("请检查您的支付宝是否填写？", '/index.php?s=/Change/changedata');
        }
        $fee = M('fee');
    ...
    private function autoMenberLock($PTid = 0)
    {
        //锁定会员
        $fck = M('fck');
        $cash = M('cash');
                ...
                foreach ($rs_cash as $too) {
                    if ($too['type'] == 0) {
                        ...
                        if ($too['is_pay'] == 1 && $too['is_done'] == 0) {
                            ...
                            $rs = $s_cash = $cash->where("id in (" . $too['match_id'] . ")")->setField($varray);
                            //清空自己
                            $cash->execute("update __TABLE__ set sid=0,s_user_id=0,is_pay=0,is_cancel=1,pdt=0,match_id=0,match_num=0 where id=" . $too['id']);
                        }
                    } else {
    ...
    {
        $account = $this->isAdmin($_POST['account']);
    ...
    {
        if(!IS_AJAX){
            $this->ajaxReturn("", '非法的请求方式', 500);
        }
        $phone = I('phone');
        $userIsExist = D('Fck')->userIsExistByPhone($phone);
        if($userIsExist){
            $this->ajaxReturn("", '手机号已存在', 403);
        }
        $code = getRandChar();
        $ip = get_real_ip();
        $map['phone'] = $phone;
        ...
        $selectDbIsCanSendByIp = M("phone_sms")->where("ip = '" . $ip . "' AND create_time > " . $startTime . " AND create_time <=" . $endTime)->select();
        $selectDbIsCanSendByPhone = M("phone_sms")->where("phone = " . $phone . " AND create_time > " . $startTime . " AND create_time <=" . $endTime)->select();
        if ((count($selectDbIsCanSendByPhone) >= 6) || (count($selectDbIsCanSendByIp) >= 6)) {
            $this->ajaxReturn("", '您请求次数太多,请稍后重试', 403);
        }
        $content = "您好，您的验证码：" . $code . "，如果不是本人操作，请忽略";
        //$url="http://utf8.sms.webchinese.cn/?Uid=$uid&Key=$key&smsMob=$phone&smsText=$content";
        //$url = "http://115.28.172.169:8888/sms.aspx?action=send&userid=51&account=JACK&password=JAck1234&mobile=" . $phone . "&content=" . $content . "&sendTime=&extno=";
        //$res = file_get_contents($url);
        $res = $this->send_sms_new($content, $phone);
        $xmlSendRes = simplexml_load_string($res);
        $sendStatus = (string)$xmlSendRes->returnstatus;
        $sendCount = (int)$xmlSendRes->successCounts;
        if ($sendStatus == "Success" && $sendCount > 0) {
            $_SESSION['Mcode'] = $code;
            session("reg_phone", $phone);
            $this->ajaxReturn("", '', 0);
        } else {
            $this->ajaxReturn("", '发送失败,请重试', 400);
        }
    }
}
?>
