...
//		}
		...
		if ($rs2){
			...
			$cash->commit();
			$bUrl = __URL__.'/eb_sell';
			$this->_box(1,xstr('hint030'),$bUrl,1);
			exit;
		}else{
			//事务回滚：
			$cash->rollback();
			$this->error(xstr('hint031'));
			exit;
		}
	}
...
//		$map['uid'] = array('neq',$id);//不能购买自己的
		...
		if($rs){
			...
			if($resute!=false){
				...
				$bUrl = __URL__.'/eb_list_b/';
				$this->_box(1,xstr('hint032'),$bUrl,1);
				exit;
			}else{
				$this->error(xstr('hint033'));
				exit;
			}
	...
	public function eb_list_AC(){
		...
		if($rs){
			...
			if($resute!=false){
				$bUrl = __URL__.'/eb_list_b';
				$this->_box(1,xstr('hint034'),$bUrl,1);
				exit;
			}else{
				$this->error(xstr('hint035'));
				exit;
			}
		...
		if($rs){
			...
			if($result){
				...
				if($s_type==1){
					...
					$fck->query('update __TABLE__ set agent_cash=agent_cash+'.$rs['money'].' where id='.$rs['uid']);
				}
	...
	public function eb_list_done(){
		...
		if($rs){
			...
			if($result&&$result2){
				...
				$bUrl = __URL__.'/eb_list';
				$this->_box(1,xstr('hint039'),$bUrl,1);
				exit;
			}else{
				//事务回滚：
				$fck->rollback();
				$this->error(xstr('hint040'));
				exit;
			}
	...
	public function eb_list_Nook(){
		...
		if($rs){
			...
			if($resute&&$xxid){
				...
				$bUrl = __URL__.'/eb_list';
				$this->_box(1,xstr('hint037'),$bUrl,1);
				exit;
			}else{
				//事务回滚：
				$fck->rollback();
			...
			if($rs){
				...
				if($stype==1){
					...
					if($s_type==1){
						...
						$result = $fck->execute("update __TABLE__ set agent_cash=agent_cash+".$cmoney." where id=".$selluid);
					}
					...
					if($s_type==1){
						...
						$result = $fck->execute("update __TABLE__ set agent_cash=agent_cash+".$cmoney." where id=".$buyuid);
					}
				...
				if($result){
					...
					$this->_box(1,xstr('hint038'),$bUrl,1);
					exit;
				}else{
					$this->error(xstr('comput_failed'));
					exit;
				}
			}else{
				$this->error(xstr('data_error'));
