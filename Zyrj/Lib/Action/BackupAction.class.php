...
class BackupAction extends CommonAction {
	...
	public function index(){
		...
		$this->distheme('index',$v_title[107]);
	}
	...
	public function indexAC(){
		...
		if ($action == xstr('delete')) {
			$this->index_del();
		}
		if ($action == xstr('restore')) {
			$this->DBHuanYuan();
		}
		if ($action == xstr('download')) {
//			$this->DBDoZip();
			$path = $_POST['mname'];
			$backulr = __URL__."/DBDoZip/mname/".$path;
			echo "<script>window.location='".$backulr."';</script>";
			echo xstr('hint011')."[<a href='".__URL__."'>".xstr('go_back')."</a>]";
			exit;
		}
...
//		$web_path = substr($web_path,1);
		...
		if($errmsg){
			$this->error(xstr('hint012').$errmsg);
			exit;
		}else{
			$bUrl = __URL__.'/index/';
			$this->success(xstr('delete_success'));
			exit;
		}
	}
	...
	public function deldir($dir,&$errmsg){
		...
		if(rmdir($dir)){
			...
			$errmsg .= "<li>".$dir.xstr('delete_failed')."</li>";
		}
	}
	...
	public function sql_query($dir="",&$errmsg="",&$msg="",$n=0,$maxt=0){
		...
		if ($err>0){
			...
			$data .= "-- ".$err.xstr('hint013')." --\r\n".$mssg;

			$date = date("Y-m-d H-i-s");
			$ctname = "error ".$date;
			$this->createtable_txt($ctdir,$ctname,$data,$err,1);
			$wronglog = $ctdir."/".$ctname.".log";
			$errmsg = xstr('hint014').$wronglog.' 。';
		}else{
			$msg = xstr('restore_success');
		}
