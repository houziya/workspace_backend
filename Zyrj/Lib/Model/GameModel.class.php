...
class GameModel extends CommonModel {
   ...
   public function clearTimeoutCash(){
		...
		$this->execute("update __TABLE__ set used_money=g_money where get_time<=$t and used_money<g_money");//这条记录没有可以使用的积分了
	...
	public function updateGameCash($uid=0,$gm=0){
   		if(!empty($_SESSION[C("USER_AUTH_KEY")])){
			...
			foreach($rs as $vo){
				...
				if($lm <= $cgm){
					$this->execute("update __TABLE__ set used_money=g_money where id=$gid");//使用的钱比这条记录的钱大，那么这个积分就使用完了
					$cgm	= $cgm - $lm;//余留的钱
				}else{
					$this->execute("update __TABLE__ set used_money=used_money+$cgm where id=$gid");//还有可以使用的积分
					$cgm	= 0;//余留的钱
				}
			}
		}
