function bank_us(name) {
    ...
    if (name == '财付通') {
        bank.innerHTML = 'Account number'
    } else {
        bank.innerHTML = 'Bank card'
    }
}
...
function yhPass(Pass)
{
	...
	if(E_Name)
	{
		...
		if(DvFul){DvFul.innerHTML = "<div class='msg_error'>The contents of the input must be greater than one character!</div>";}
		return false;
	}
	return false;
}
	...
	if(E_Name && re_Name)
	{
		...
		//{
			if(E_Name.value == re_Name.value){
				...
				if(DvFul){DvFul.innerHTML = "<div class='msg_error'>Input and confirm the entry is inconsistent!</div>";}
				return false;
			}
		//}
...
{ 
	...
	if (Over_Name)
	{
		...
		if (DvName){DvName.innerHTML= "<div class='msg_error'>Content can not be empty!</div>";}
		return false;
	}
	return false;
}
...
function Null_Int(Null_N)
{
	...
	if (Null_Name)
	{
		...
		if (!str == "")//判断是否为空
		{
			...
			if(IntDvName){IntDvName.innerHTML="<div class='msg_error'>I'm sorry, please fill out figures!</div>";}
			return false;
		}
		if (IntDvName){IntDvName.innerHTML= "<div class='msg_error'>Content can not be empty!</div>";}
		return false;
	}
	return false;
}
...
function Email(sMsgId)
{
	...
	if(oEmail){
		...
		if(oEmail.value){
			...
			if(oMsg){oMsg.innerHTML = "<div class='msg_error'>E-mail format is incorrect!</div>";}
			return false;
		}
	}
	return false;
